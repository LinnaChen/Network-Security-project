...
int aesEncrypt(const uint8_t *key, uint32_t keyLen,  uint8_t *pt, uint8_t *ct, uint32_t len) {
    ...
    uint8_t state[4][4] = {0};
	uint8_t I_V[16] = {0x60,0xef,0x17,0x10,0xd7,0xcc,0x28,0xf8,0x56,0xbd,0xe4,0x8b,0xa1,0xce,0xb0,0x87};
...
int aesDecrypt(const uint8_t *key, uint32_t keyLen,  uint8_t *ct, uint8_t *pt, uint32_t len) {
    ...
    keyExpansion(actualKey, 16, &aesKey);  //秘钥扩展，同加密
       ...
       for (int i = 0; i < len; i += BLOCKSIZE) {
		...
		storeStateArray(state, store);
		for(int n= 0; n < 16; n++){
		store[n] = I_V[n] ^ store[n]; 
		/*printf("%x ",pt[n]);*/
	}
		loadStateArray(state,store);
		
		storeStateArray(a, I_V);
...
void read_file(FILE *fp){
	...
	*/
	...
	i = strlen(temp);
	if(i%2 != 0){
		printf("The Length of Your Text is Odd Number!");
...
int main() {
    ...
    uint8_t pt[64]={0x00,0x11,0x22,0x33,0x44,0x55,0x66,0x77,0x88,0x99,0xaa,0xbb,0xcc,0xdd,0xee,0xff,0x00,0x11,0x22,0x33,0x44,0x55,0x66,0x77,0x88,0x99,0xaa,0xbb,0xcc,0xdd,0xee,0xff,0x00,0x11,0x22,0x33,0x44,0x55,0x66,0x77,0x88,0x99,0xaa,0xbb,0xcc,0xdd,0xee,0xff,0x00,0x11,0x22,0x33,0x44,0x55,0x66,0x77,0x88,0x99,0xaa,0xbb,0xcc,0xdd,0xee,0xff};
    uint8_t ct[64] = {0};     // 外部申请输出数据内存，用于加密后的数据
    uint8_t plain[64] = {0};  // 外部申请输出数据内存，用于解密后的数据
	uint8_t I_V[16] = {0x60,0xef,0x17,0x10,0xd7,0xcc,0x28,0xf8,0x56,0xbd,0xe4,0x8b,0xa1,0xce,0xb0,0x87};
	...
	printHex(pt, 64, "plain data:"); // 打印初始明文数据


    aesEncrypt(key, 16, pt, ct, 64); // 加密
    
    //printf("expect cipher:\n39 25 84 1D 02 DC 09 FB DC 11 85 97 19 6A 0B 32\n");  // 正常解密后的数据内容

    printHex(ct, 64, "after encryption:");  // 打印加密后的密文

    aesDecrypt(key, 16, ct, plain, 64);       // 解密
    printHex(plain, 64, "after decryption:"); // 打印解密后的明文数据
